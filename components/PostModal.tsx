    ...
    <div className="fixed inset-0 bg-black/85 backdrop-blur-[8px] z-[2000] flex items-center justify-center p-5 animate-[modalFade_0.3s_cubic-bezier(0.16,1,0.3,1)]">
      <div className="w-full max-w-[700px] bg-[var(--primary)] border border-[var(--border)] rounded-[24px] max-h-[85vh] overflow-y-auto p-10 px-7.5 relative">
        ...
        <div className="flex flex-col sm:flex-row gap-3 mt-8 justify-center items-center">
          {/* 링크생성, PDF출력: 임시 전면 개방 */}
          <button className="w-full sm:w-auto sm:flex-1 sm:max-w-[160px] p-3.5 rounded-xl border border-[var(--border)] bg-[var(--card-bg)] text-[var(--text-main)] text-[0.85rem] font-bold cursor-pointer flex items-center justify-center gap-2 transition-all hover:border-[var(--accent)]" onClick={() => onCopyLink(post.id)}>
          ...
          <button className="w-full sm:w-auto sm:flex-1 sm:max-w-[160px] p-3.5 rounded-xl border border-[var(--border)] bg-[var(--card-bg)] text-[var(--text-main)] text-[0.85rem] font-bold cursor-pointer flex items-center justify-center gap-2 transition-all hover:border-[var(--accent)]" onClick={() => onPrintPDF(post)}>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
